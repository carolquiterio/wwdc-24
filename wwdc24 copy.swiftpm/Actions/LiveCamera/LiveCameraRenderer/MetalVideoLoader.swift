...
public class MetalVideoLoader: OperationChain {
    ...
    public convenience init(url: URL, playAtActualSpeed: Bool = true, loop: Bool = true, sourceKey: String = "video") throws {
        let asset = AVURLAsset(url: url, options: [AVURLAssetPreferPreciseDurationAndTimingKey: true])
        try self.init(asset: asset, playAtActualSpeed: playAtActualSpeed, loop: loop)
    }

    public init(asset: AVAsset, playAtActualSpeed: Bool = true, loop: Bool = true, sourceKey: String = "video") throws {
        self.asset = asset
        self.loop = loop
        ...
        asset.loadValuesAsynchronously(forKeys: ["tracks"]) {
            DispatchQueue.global().async {
                ...
                guard self.assetReader.startReading() else {
                    debugPrint("error when start reading")
                    return
                }
